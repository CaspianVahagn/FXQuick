...
public class FXInitialization {
    ...
    private static void scanServices(String... packageNames) {
        FXInitialization.packageNames = packageNames;
    }
    ...
    private static void scanRuntimeInjections(String... packageNames) {
        ...
        FXInitialization.potentialInjections = packageNames;
    }

    private static void addViewNameSpaces(String... packageNames) {
        FXInitialization.viewPackages = packageNames;
    }
    ...
    private static void InitSerivesAndController(Reflections refs) {
        Set<Class<?>> annotatedClasses = refs.getTypesAnnotatedWith(FXConfiguration.class);
        annotatedClasses.addAll(refs.getTypesAnnotatedWith(FXService.class));
        annotatedClasses.addAll(refs.getTypesAnnotatedWith(FXFeignClient.class));
        List<Class<?>> listOfAnnotations = annotatedClasses.stream().collect(Collectors.toList());
        listOfAnnotations.sort((aClass, t1) -> {
                if(aClass.isInterface() && aClass.isInterface()) return 0;
                if (aClass.isInterface()) return -1;
                if (t1.isInterface()) return 1;
    ...
    private static void InitialitializingWithConstructor(List<Class<?>> listOfAnnotations, List<Class<?>> initLater) {
        for (Class<?> clazz : listOfAnnotations) {
            if (Modifier.isInterface(clazz.getModifiers())&& clazz.isAnnotationPresent(FXFeignClient.class)) {
                ...
                try {
                    ...
                    if(client.config() != FXFeignClient.class){
                        if(ServiceManager.CLASSES.containsKey(client.config())){
                            o = initializeBuiltFeignClient(clazz, client,(FXFeignConfig) ServiceManager.CLASSES.get(client.config()));
                        }else{
                            initLater.add(clazz);
                            continue;
                        }
            ...
            if (!Modifier.isAbstract(clazz.getModifiers())) {
                ...
                try {
                    ...
                    for(Class<?> aClass: constructor.getParameterTypes()){
                        if(aClass.isAnnotationPresent(FXService.class) || aClass.isAnnotationPresent(FXConfiguration.class) || aClass.isAnnotationPresent(FXFeignClient.class)){
        ...
        for (Class<?> clazz : annotatedClients) {
            ...
            if (Modifier.isInterface(clazz.getModifiers())) {
                ...
                try {
                    ...
                    if(client.config() != FXFeignClient.class){
                        o = initializeBuiltFeignClient(clazz, client, null);
                    }else {
                        o = Feign.builder()
                                .client(new OkHttpClient())
                                .encoder(new JacksonEncoder())
    ...
    private static Object initializeBuiltFeignClient(Class<?> clazz, FXFeignClient client, FXFeignConfig initializedConfig) throws InstantiationException, IllegalAccessException, InvocationTargetException, NoSuchMethodException {
        Object o;
        FXFeignConfig config = initializedConfig;
